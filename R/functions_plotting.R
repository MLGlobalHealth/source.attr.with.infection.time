...
make_plot_simulated_data_colour_prob_tpair_sizes <- function(po,dps_clock,sim_scenario,outfile.base){
  ...
  p <- ggplot(data = dps_clock,aes(x=d_TSeqT))+
    ...
    theme_bw(base_size=16)+geom_point(data=po,aes(x=TIME_ELAPSED,y=GEN_DIST,size=M,colour=TRANSMISSION_PAIR),alpha=0.8)+
    scale_color_manual(values=c(pal_npg('nrc')(9)[8],pal_npg('nrc')(9)[2])) +
    ...
    labs(x='Time elapsed (in years)',y='\nPatristic distance of\nphylogenetically possible pair',color="True transmission pair",size='Transmission pair\nprobability under model\n(posterior median)')+
    theme(legend.position='bottom',
          legend.title=element_text(size=rel(0.7)),
          legend.text=element_text(size=rel(0.7)))+
    scale_x_continuous(expand = c(0,0)) +
