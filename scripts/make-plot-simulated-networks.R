...
p <- ggplot(data = dps,aes(x=d_TSeqT))+
  ...
  geom_hline(yintercept=0.015,linetype='dashed',size=1,col='black') +
...
p
ggsave(file=file.path(out.dir,'simulated_data_after_exclusions_falsepos.png'), p, w=7, h=5)


g <- ggarrange(p_hist+labs(x='Phylogenetically plausible sources per case')+guides(fill='none'),p,nrow=2,align='hv',heights=c(0.5,0.5)
               ,               labels=c('B','C'),font.label=list(size=20),vjust=1)
ggsave(file=file.path(out.dir,'simulated_data_clustersize_after_exclusions_falsepos_y20pct.png'), g, w=7.4, h=9)
ggsave(file=file.path(out.dir,'simulated_data_clustersize_after_exclusions_falsepos_y20pct.pdf'), g, w=7.4, h=9)
