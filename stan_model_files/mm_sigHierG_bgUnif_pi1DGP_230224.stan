...
transformed data
{
  ...
  matrix[Nux, M[1]] hsgp_eigenvalues = PHI(N, M[1], L[1], ux);
}
...
transformed parameters
{
  ...
  vector[N] tpair_beta;
  vector[M[1]] hspg_spectral_dens;
  vector[N] hsgp_f;

  // spectral densities for f
  hspg_spectral_dens = diagSPD_EQ(gpscale, lscale, L[1], M[2]);

  // HSGP approximation to f at inputs
  hsgp_f = (hsgp_eigenvalues * (hspg_spectral_dens .* hsgp_beta))[x_2_ux];

  // mixing weights
  y_mix = inv_logit(logit_y_mix_0 + hsgp_f);
